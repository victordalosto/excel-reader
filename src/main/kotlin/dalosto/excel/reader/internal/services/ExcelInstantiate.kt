...
private fun <T> setFieldValue(instance: T, field: ExcelFields, cell: Cell?) {
    ...
    if (field.annotation.required && (cell?.value == null || cell.type == CellType.EMPTY)) {
        throw ExcelReaderException("Required field '${field.annotation.headerName}' is missing.")
    }
        ...
        if (fieldType != String::class.java && cell.value is String) {
            ...
            if (value.isBlank() || value in EXCEL_UNDEFINED_VALUES) {
                if (field.annotation.required) {
                    throw ExcelReaderException(
                        "Required field '${field.annotation.headerName}' " +
                        "is missing in the row at column index ${cell.columnIndex}."
                    )
                }
