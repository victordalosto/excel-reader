...
internal fun booleanValue(cell: Cell): Boolean {
    return when (cell.type) {
        ...
        CellType.STRING -> {
            when (val value = (cell.value as String).trim().lowercase()) {
                in TRUE_VALUES -> true
...
internal fun numberValue(cell: Cell): BigDecimal {
    return when (cell.type) {
        ...
        CellType.STRING -> cleanNumberString(cell.value as String)
...
private fun cleanNumberString(input: String): BigDecimal {
    try {
        var value = input.uppercase()
                         .replace("\\s".toRegex(), "")
                         .replace(",", ".")
                         .replace("10^", "E")
